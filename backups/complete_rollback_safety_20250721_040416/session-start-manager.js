...
const SessionStartManager = {
    ...
    async startSession() {
        ...
        // 🎨 Phase B-3: セッション開始時の左パネル動的表示システム確実初期化
        if (window.DynamicPanelManager && !window.DynamicPanelManager.isInitialized) {
            console.log('🔄 セッション開始時に左パネル初期化実行...');
            try {
                const panelInitialized = window.DynamicPanelManager.initialize();
                if (panelInitialized) {
                    window.DynamicPanelManager.isInitialized = true;
                    console.log('✅ セッション開始時の左パネル初期化成功');
                } else {
                    console.warn('⚠️ セッション開始時の左パネル初期化失敗');
                }
            } catch (error) {
                console.error('❌ セッション開始時の左パネル初期化エラー:', error);
            }
        } else if (window.DynamicPanelManager?.isInitialized) {
            // 既に初期化済みの場合は強制更新
            console.log('🔄 左パネル表示を強制更新中...');
            if (typeof window.DynamicPanelManager.updateDisplay === 'function') {
                window.DynamicPanelManager.updateDisplay(true);
            }
        }
    ...
    async startWarmupPhase() {
        ...
        try {
            // 🎤 セッション開始時の音声認識初期化（統合版）
            console.log('🎤 セッション開始 - 音声認識システム確認');
            
            // 1. StateManagerの初期化確認と開始
            ...
            // 2. StateManagerによる音声認識開始
            let voiceStarted = false;
            if (window.stateManager) {
                try {
                    const started = await window.stateManager.startRecognition();
                    if (started) {
                        console.log('✅ StateManagerで音声認識を開始しました');
                        voiceStarted = true;
                    }
                } catch (error) {
                    console.warn('⚠️ StateManagerでの音声認識開始に失敗:', error);
                }
            }
            
            // 3. フォールバック: 統一状態管理システム
            if (!voiceStarted && window.unifiedStateManager) {
                try {
                    const voiceModule = window.unifiedStateManager.getModule('voice');
                    if (voiceModule) {
                        const started = await voiceModule.startRecognition();
                        if (started) {
                            console.log('✅ 統一状態管理システムで音声認識を開始しました');
                            voiceStarted = true;
                        }
                    }
                } catch (error) {
                    console.warn('⚠️ 統一状態管理システムでの音声認識開始に失敗:', error);
                }
            }
            
            // 4. 最終フォールバック: 旧システム
            if (!voiceStarted && typeof safeStartSpeechRecognition === 'function') {
                try {
                    const started = safeStartSpeechRecognition('session_start');
                    if (started) {
                        console.log('✅ 旧システムで音声認識を開始しました');
                        voiceStarted = true;
                    }
                } catch (error) {
                    console.warn('⚠️ 旧システムでの音声認識開始に失敗:', error);
                }
            }
            
            if (!voiceStarted) {
                console.error('❌ すべての音声認識システムの開始に失敗しました');
                window.showMessage('warning', '音声認識の開始に失敗しました。手動で再試行してください。');
            }
            ...
            // 🎤 挨拶後の音声認識開始（統合版）
            console.log('🎤 挨拶後 - 音声認識再開確認');
            
            let postGreetingVoiceStarted = false;
            
            // 1. StateManagerによる音声認識開始
            if (window.stateManager) {
                try {
                    await window.stateManager.startRecognition();
                    console.log('✅ StateManagerで挨拶後の音声認識開始完了');
                    postGreetingVoiceStarted = true;
                } catch (error) {
                    console.warn('⚠️ StateManagerでの挨拶後音声認識開始に失敗:', error);
                }
            }
            
            // 2. フォールバック: 統一状態管理システム
            if (!postGreetingVoiceStarted && window.unifiedStateManager) {
                try {
                    const voiceModule = window.unifiedStateManager.getModule('voice');
                    if (voiceModule) {
                        await voiceModule.startRecognition();
                        console.log('✅ 統一状態管理システムで挨拶後の音声認識開始完了');
                        postGreetingVoiceStarted = true;
                    }
                } catch (error) {
                    console.warn('⚠️ 統一状態管理システムでの挨拶後音声認識開始に失敗:', error);
                }
            }
            
            // 3. 最終フォールバック: 旧システム
            if (!postGreetingVoiceStarted && typeof safeStartSpeechRecognition === 'function') {
                try {
                    safeStartSpeechRecognition('post_greeting');
                    console.log('✅ 旧システムで挨拶後の音声認識開始完了');
                    postGreetingVoiceStarted = true;
                } catch (error) {
                    console.warn('⚠️ 旧システムでの挨拶後音声認識開始に失敗:', error);
                }
            }
            
            if (!postGreetingVoiceStarted) {
                console.warn('⚠️ すべてのシステムでの挨拶後音声認識開始に失敗');
            }
        } catch (error) {
