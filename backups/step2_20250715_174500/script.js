...
const MIGRATION_CONFIG = {
    USE_NEW_TRANSCRIPT: false,          // 初期は旧システム（transcript-compact）
...
window.emergencyRollback = function() {
    ...
    // 新システムを無効化
    MIGRATION_CONFIG.USE_NEW_TRANSCRIPT = false;
    ...
    if (transcriptCompact) {
        transcriptCompact.style.display = 'flex';
        console.log('✅ transcript-compact 復元完了');
    }
    
    if (transcriptDisplay) {
        transcriptDisplay.style.display = 'none';
        console.log('✅ transcript-display 無効化完了');
    }
...
window.switchToNewTranscript = function() {
    ...
    try {
        ...
        if (transcriptCompact) {
            transcriptCompact.style.display = 'none';
            console.log('✅ transcript-compact 無効化完了');
        }
...
function initializeMigrationSystem() {
    ...
    // 初期状態の設定
    if (MIGRATION_CONFIG.USE_NEW_TRANSCRIPT) {
        console.log('🎯 新システムを使用開始');
        window.switchToNewTranscript();
    } else {
        console.log('🎯 既存システムを維持');
        // 既存システムが確実に動作するように
        if (transcriptCompact) {
            transcriptCompact.style.display = 'flex';
        }
