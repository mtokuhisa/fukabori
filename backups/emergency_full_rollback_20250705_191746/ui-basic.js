...
function updateVoiceSettingsUI() {
    // AI_PROMPTSの読み込み状況を確認
    if (typeof window.AI_PROMPTS === 'undefined') {
        console.log('⚠️ AI_PROMPTSが未読み込み - 音声設定UIの更新をスキップ');
        return;
    }
    
    // VOICE_SETTINGSが存在しない場合はデフォルト設定を使用
    const voiceSettings = window.AI_PROMPTS.VOICE_SETTINGS || {
        nehori: { voice: 'ja-JP-NanamiNeural', speed: 1.0, pitch: 1.0 },
        hahori: { voice: 'ja-JP-KeitaNeural', speed: 1.0, pitch: 1.0 }
    };
...
function update2StepUI() {
    ...
    if (step1Checkbox) {
        step1Checkbox.textContent = hasApiKey ? '✅' : '❌';
        step1Checkbox.style.color = hasApiKey ? '#4caf50' : '#f44336';
    }
    ...
    if (step2Checkbox) {
        step2Checkbox.textContent = hasTheme ? '✅' : '❌';
        step2Checkbox.style.color = hasTheme ? '#4caf50' : '#f44336';
    }
    
    // セッション開始ボタンの状態は既存のupdateSessionStartButton関数を使用
    const allComplete = hasApiKey && hasTheme;
    if (typeof window.updateSessionStartButton === 'function') {
        window.updateSessionStartButton(allComplete);
    }
