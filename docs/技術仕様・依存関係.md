...
20. app/voice-ui-manager.js      - 音声UI管理
21. app/voice-error-handler.js   - 音声エラーハンドラー
...
#### 音声認識システム
```
ContinuousRecognitionManager
├── depends on: VoiceErrorHandler, VoiceUIManager
├── initializes: 音声認識基盤、エラーハンドラー、UI管理
└── provides: 継続的音声認識、状態管理、エラー回復
...
VoiceUIManager
├── depends on: DOM操作、状態管理
├── provides: 音声UI状態表示、ビジュアルフィードバック
└── functions: 状態別メッセージ表示、色変更、アイコン制御
```
...
case 'no-speech':
    ...
    this.isNoSpeechRecovery = true;
    // 不正終了を防ぐフラグ設定
    return; // エラー処理をスキップ
```

#### ignoreNextEndフラグによる不正終了防止
```javascript
// 問題の流れ
