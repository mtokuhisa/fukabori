...
(function(global) {
    ...
    class VoiceProcessingManager {
        ...
        async processFinalTranscript(text) {
            if (!this.isInitialized) {
                this.log('warn', '未初期化状態 - 従来システムに委譲');
...
// 🔧 二重処理システム単純化完了 - 直接委譲方式に変更
        
        /**
         * 音声認識継続性の保証
         */
        ensureVoiceRecognitionContinuity(context = 'unknown') {
            this.log('info', `🔧 音声認識継続性チェック開始: ${context}`);
            ...
            try {
                ...
                if (window.stateManager && window.stateManager.recognitionManager) {
                    ...
                    this.log('info', `🔍 現在の音声認識状態: ${currentState}`);
                    
                    if (currentState === 'idle' || currentState === 'error') {
                        this.log('warn', `🚨 音声認識が停止状態: ${currentState} - 復旧開始`);
                        
                        // 短い遅延の後に復旧実行
                        setTimeout(() => {
                            if (window.AppState?.sessionActive && recognitionManager.state !== 'active') {
                                this.log('info', `🔄 音声認識復旧実行: ${context}`);
                                recognitionManager.start().catch(error => {
                                    this.log('error', `音声認識復旧失敗: ${error.message}`);
                                });
                            }
                        }, 500);
                    } else if (currentState === 'active') {
                        this.log('info', `✅ 音声認識は正常に継続中: ${currentState}`);
                    }
                } else {
                    this.log('warn', '⚠️ 統合音声システム未初期化');
                }
                
            } catch (error) {
                this.log('error', `音声認識継続性チェックエラー: ${error.message}`);
            }
        }
        
// 🔧 処理単純化完了 - エラー時は従来システム直接委譲
        ...
        detectDeleteCommand(text) {
            ...
            // Phase 3.2: 簡易な文字数削除（数値指定） - 「してください」対応強化
            const numberDeletePatterns = [
                /(\d+)文字\s*消して(?:ください)?/i,      // "3文字消して", "3文字消してください"
                /(\d+)文字\s*削除(?:して(?:ください)?)?/i, // "3文字削除", "3文字削除して", "3文字削除してください"
                /最後の(\d+)文字\s*削除(?:して(?:ください)?)?/i, // "最後の3文字削除", "最後の3文字削除してください"
                /最後の(\d+)文字\s*消して(?:ください)?/i,     // "最後の3文字消して", "最後の3文字消してください"
                /(\d+)文字\s*戻して(?:ください)?/i,           // "3文字戻して", "3文字戻してください"
                /(\d+)文字\s*取り消し(?:て(?:ください)?)?/i,   // "3文字取り消し", "3文字取り消してください"
                /(\d+)\s*文字\s*消して(?:ください)?/i,        // "3 文字 消して", "3 文字 消してください"
                /(\d+)\s*文字\s*削除(?:して(?:ください)?)?/i   // "3 文字 削除", "3 文字 削除してください"
            ];
        ...
        async handleDeleteCommandWithSafety(commandData) {
            try {
                ...
                // 🎯 シンプル修正：削除処理前にtranscriptから音声コマンド部分を除去
                if (commandData.type === 'number_delete' && window.removeVoiceCommand) {
                    // 実際のcurrentTranscriptから音声コマンドを除去
                    if (window.AppState?.currentTranscript) {
                        ...
                        this.log('info', `🔍 デバッグ: "${currentTranscript}" → "${cleanedText}"`);
                        
                        if (cleanedText !== currentTranscript) {
                            // currentTranscriptを更新
                            window.AppState.currentTranscript = cleanedText;
                            
                            // transcriptHistoryの完全更新：音声コマンド除去後の状態に統一
                            if (cleanedText.trim()) {
                            ...
                            // 🎯 重要：音声コマンド除去済みテキストをcommandDataに保存
                            commandData.cleanedText = cleanedText;
                            
                            this.log('info', `🎯 音声コマンド除去完了: "${currentTranscript}" → "${cleanedText}"`);
                            this.log('info', `📝 transcriptHistory更新: [${window.AppState.transcriptHistory.join(', ')}]`);
                            this.log('info', `💾 cleanedTextをcommandDataに保存: "${cleanedText}"`);
        ...
        async handleNumberDelete(commandData) {
            try {
                ...
                // 🎯 修正：確認処理を無効化して全て即座実行
                this.log('info', `確認処理をスキップして即座実行: ${commandData.count}文字削除`);
                
                // waitingForPermissionを一時的にリセット
                ...
                try {
                    // 🎯 修正：事前に除去・保存された音声コマンド除去済みテキストを使用
                    const cleanedTextForDeletion = commandData.cleanedText || null;
                    
                    if (cleanedTextForDeletion) {
                        this.log('info', `🔧 保存された音声コマンド除去済みテキストを使用: "${cleanedTextForDeletion}"`);
                    } else {
                        this.log('info', `⚠️ 音声コマンド除去済みテキストが見つからないため、従来方式で処理`);
                    }
                    ...
                    // 成功メッセージ
                    // 🎯 軽量化: 削除時の音声フィードバックも削除（処理軽量化のため）
                    
                    this.log('info', `数値指定削除完了: ${result.deletedCount}文字削除`);
                    
                    // 🎯 重要：削除処理完了後の状態完全同期
                    if (window.AppState && window.SpeechCorrectionSystem) {
                        const finalText = window.SpeechCorrectionSystem.getCurrentInput();
                        
                        // transcriptHistoryとcurrentTranscriptの完全同期
                        if (finalText) {
                        ...
                        // state-integration-adapter.jsに変更を通知
                        ...
                        this.log('info', `🔄 削除処理後の状態同期完了: transcriptHistory=[${window.AppState.transcriptHistory.join(',')}], currentTranscript="${window.AppState.currentTranscript}"`);
                    }
                    
                    // ✅ 正しいアプローチ: 「どうぞ」待機状態の確実な設定
                    if (window.AppState) {
                        window.AppState.waitingForPermission = true;
                        this.log('info', `⏸️ 「どうぞ」待機状態に設定: 削除完了後は必ず「どうぞ」が必要`);
                    }
                    ...
                    // ✅ 削除処理完了後は「どうぞ」待機状態を維持（音声認識は継続、会話は「どうぞ」を待つ）
                    if (window.AppState) {
                        window.AppState.waitingForPermission = true;
                        this.log('info', `✅ 「どうぞ」待機状態を設定: 削除完了後は「どうぞ」が必要`);
        ...
        async executeNumberDelete(count, cleanedText = null) {
            try {
                // 🎯 修正：音声コマンド除去済みテキストを使用（渡されている場合）
                let sourceText;
                
                if (cleanedText !== null) {
                    // 音声コマンド除去済みテキストが渡された場合はそれを使用
                    sourceText = cleanedText;
                    this.log('info', `🔧 音声コマンド除去済みテキストを削除処理に使用: "${sourceText}"`);
                } else {
                    // 従来通り：transcriptHistoryとcurrentTranscriptから構築
                    ...
                    this.log('info', `🔧 従来方式でテキスト構築: "${sourceText}"`);
                }
                ...
                this.log('info', `🎯 削除実行: "${sourceText}"(${sourceText.length}文字) → "${newText}"(${newText.length}文字), ${deleteCount}文字削除`);
